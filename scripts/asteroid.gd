...
func _ready():
	...
	randomize()
	set_process(true)
	vel=Vector2(rand_range(20,100), 0).rotated(rand_range(0,2*PI))
	rot_speed= rand_range(-1.5,1.5)
	screen_size=get_viewport_rect().size
	#init('big',screen_size/2)
...
func init(init_size,init_pos, init_vel):
	...
	extents=texture.get_size()/2 #Исключительно эстетическое деление - чем меньше extentsтем раньше перенсоится на другую сторону
